...
NS_SDL_BEGIN
Node::Node()
	: _name(""), _tag(-1), _parent(NULL), _visible(true), _localZOrder(0), _globalZOrder(0.f), _reorderChildDirty(false), _running(false), _position(Point::ZERO), _contentSize(Size::ZERO), _rotation(0.f), _scaleX(1.f), _scaleY(1.f), _anchorPoint(0.5f, 0.5f), _flipX(false), _flipY(false), _realOpacity(255), _displayedOpacity(255), _cascadeOpacity(false), _colorMod(255, 255, 255), _cascadeScale(false), _userdata(nullptr), _userObject(nullptr), _eventDispatcher(nullptr), _actionManager(nullptr), _scheduler(nullptr)
{
...
void Node::scheduleUpdate()
{
	_scheduler->schedule([&](float dt)
						 { this->update(dt); },
						 this, "update" + std::to_string(_uniqueID), 0, SDL_REPEAT_FOREVER, 0, !_running);
}
void Node::unscheduleUpdate()
{
	_scheduler->unschedule(this, "update" + std::to_string(_uniqueID));
}
