...
EventDispatcher::EventDispatcher()
	: _inDispatch(false), _reorderListenersDirty(false)
{
}
EventDispatcher::~EventDispatcher()
{
...
void EventDispatcher::removeEventListener(EventListener *listener)
{
	...
	auto removeListenerInVector = [&](std::vector<EventListener *> *listeners) -> bool
	{
		...
		if (iter != listeners->end())
		{
			...
			{
				this->dissociateNodeAndEventListener(listener->getAssociatedNode(), listener);
...
void EventDispatcher::removeEventListenerForTarget(Node *node, bool recursive)
{
...
void EventDispatcher::resumeEventListenersForTarget(Node *node, bool recursive)
{
...
void EventDispatcher::pauseEventListenersForTarget(Node *node, bool recursive)
{
	SDLASSERT(node, "node should not null");
...
void EventDispatcher::updateListeners(const std::string &listenerID)
{
	auto onUpdateListeners = [this](const std::string &listenerID)
	{
		...
		for (auto iter = listeners->begin(); iter != listeners->end();)
		{
			...
			if (!listener->isRegistered())
			{
				...
				auto matchIter = std::find(_toRemovedListeners.begin(), _toRemovedListeners.end(), listener);
	...
	if (listenerID == EventListenerTouchOneByOne::LISTENER_ID || listenerID == EventListenerAllAtOnce::LISTENER_ID)
	{
		onUpdateListeners(EventListenerTouchOneByOne::LISTENER_ID);
		onUpdateListeners(EventListenerAllAtOnce::LISTENER_ID);
	}
	else if (listenerID == EventListenerKeyboard::LISTENER_ID || listenerID == EventListenerKeyboardState::LISTENER_ID)
	{
