...
public:
	SchedulerEntity(float interval, unsigned int repeat, float delay, bool paused, int priority = 0)
			: _interval(interval), _repeat(repeat), _delay(delay), _executeTimes(0), _paused(paused), _elapsed(0.f), _priority(priority), _bDeleteSelf(false)
	{
	}
	~SchedulerEntity()
	{
...
public:
	SchedulerEntityCallback(const ccScheduleCallback &callback, Node *target, const std::string &key, float interval, unsigned int repeat, float delay, bool paused, int priority = 0)
			: SchedulerEntity(interval, repeat, delay, paused, priority), _target(target), _callback(callback), _key(key)
	{
	}
	virtual void execute(float dt);
};
...
public:
	SchedulerEntitySelector(SEL_SCHEDULE selector, Object *target, float interval,
			...
			: SchedulerEntity(interval, repeat, delay, paused, priority), _target(target), _selector(selector)
	{
	}
